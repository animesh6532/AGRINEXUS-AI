...
export const WeatherScene: React.FC<WeatherSceneProps> = ({ state, children }) => {
  ...
  return (
    <div className="relative w-full rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-[#0B192C]">
      {/* Layer 0: Sky Gradient & Celestial Elements */}
      <WeatherSky condition={condition} isDay={isDay} sunVisibility={intensity.sunVisibility} />

      {/* Layer 1: Multi-layer Parallax Clouds */}
      <WeatherClouds
      ...
      {/* Layer 2: Canvas 2D Particle Engine (Rain, Splashes, Snow, Wind, Lightning) */}
      <WeatherCanvas intensity={intensity} />

      {/* Layer 3: Layered Translucent Fog Atmospheric Gradient */}
      {intensity.fogIntensity > 0.05 && (
        <div
          className="absolute inset-0 bg-gradient-to-t from-slate-300/40 via-slate-400/20 to-transparent pointer-events-none transition-opacity duration-1000 z-10"
          style={{ opacity: intensity.fogIntensity }}
        />
      )}

      {/* Layer 4: Telemetry UI Overlay & Controls */}
      <div className="relative z-20 p-6 sm:p-8 md:p-10">{children}</div>
    </div>
  );
};
